...
export const AccountOverviewScreen = ({navigation}: Props) => {
  ...
  return (
    <TemplateScreen>
      <>
        {accounts && (
          <Text selectable={true}>{Object.keys(JSON.parse(accounts))}</Text>
        )}
      </>
      <View
        style={{
        ...
        <Button title="Clear Acc" onPress={clearAccounts} />
        <Button title="Retrieve Acc" onPress={retrieveAccounts} />
      </View>
      <FlatList
        data={AccountItems}
